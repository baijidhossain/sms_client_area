...
<body>
    ...
    <script>
        ...
        const iti = intlTelInput($('[name="recipient"]')[0], {
                        ...
                        <a data-threadid="${thread.thread_id}" style="cursor: pointer;">
                            <div class="align-items-center media">
                                ...
                                <div class="media-body overflow-hidden">                                
                                    <h5 class="text-truncate font-size-15 mb-0">${thread.receiver}</h5>
                                </div>            
                            </div>
                        </a>
                    </li>`
        ...
        async function fetchThreads() {
            ...
            if (!data.threads.length) {
                try {
                    ...
                    fetchData.data ? data.threads = fetchData.data.items : false;
        ...
        async function searchThreads(term) {
            const response = await _request("/threads/search", 'GET', {
                query: term
            });
        ...
        $(function() {
            ...
            $('#searchInput').on('input', (event) => {
                ...
                //filter threads
                data.filteredThreads = data.threads.filter(thread => thread.receiver.toLowerCase().includes(searchQuery));
