                        ...
                        <form id="message_form" method="POST" enctype="multipart/form-data">
                            <div class="row g-0">
                                ...
                                <div class="col">
                                    ...
                                    <input type="file" name="file" onchange="return fileValidation()" id="upload_input" class="upload_input" style="display: none;" accept="image/jpg, image/jpeg, image/png, image/gif">
                                </div>
                                <div class="col-auto">
    ...
    <script>
        ...
        function threadsListTemplate() {
            ...
            const filteredThreads = data.threads.filter(thread => {
                if (data.filteredThreads) {
                    const searchTerm = data.filteredThreads;
                    return thread.receiver.includes(searchTerm);
                }
                return true; // If no filtering, include all threads
            });
        ...
        $('#searchInput').on('keyup', (event) => {
            data.filteredThreads = event.target.value.trim();
                            ...
                            <div class="user-chat-content">
                                <div class="ctext-wrap">
                                    ...
                                    <div class="dropdown align-self-start">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item copy-btn" href="#">Copy <i class="ri-file-copy-line float-end text-muted"></i></a>
                                            <a class="dropdown-item" href="#">Delete <i class="ri-delete-bin-line float-end text-muted"></i></a>
                                        </div>
                                    </div>
                                </div>
        ...
        function fileValidation() {
            ...
            if (!allowedExtensions.test(filePath)) { // Use test() to check a regex
                showError("Please select only valid jpg, jpeg, png image file.");
                fileInput.val(""); // Use val("") to clear the input
            } else {
                // Image preview
                if (fileInput[0].files && fileInput[0].files[0]) {
                    if (fileSize > 4194304) {
                        showError("pless select file less than 4MB");
                        fileInput.val("");
                    }
                }
            }
        ...
        async function replyMessage(e) {
            ...
            const last_msg_id = data.messages.find(item => item.thread.id == data.selectedThreadId).items[0].id;
            ...
            const resp = await _request(`/threads/reply/${data.selectedThreadId}`, 'POST', formData, hasFile);
            ...
            // update state
            const thread = data.messages.find(item => item.thread.id == data.selectedThreadId);
            ...
            thread.items.unshift({
                ...
                status: 'Pending',
                thread_id: data.selectedThreadId
            });
