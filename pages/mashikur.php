...
<body>
    ...
    <div class="layout-wrapper d-md-flex">
        ...
        <div class="side-menu flex-md-column me-lg-1 ms-lg-0">
            <!-- LOGO -->
    ...
    <script>
        ...
        async function savePhoneDataToDB(phoneData) {
            ...
            for (const [key, value] of Object.entries(phoneData.threads)) {
                ...
                for (const thread of value.items) {
                    ...
                    //add phone_id to thread
                    thread.phone_id = key;

                    //save thread
                    await phoneThreadsStore.put(thread);
                }
            }
        ...
        async function loadFromDB() {
            ...
            //loop messages and add thread_id as array key
            for (const message of messages) {
                messageWithKey[message.thread_id] = message;
            }
        ...
        async function lastMessageID() {
            ...
            return cursor.value.message_id;
        }
    ...
    <script>
        ...
        async function loadAndInitializeState() {
            ...
            try {
                ...
                if (Object.keys(localState).length !== 0) {
                    ...
                    await savePhoneDataToDB(stateObject.phoneData);
                    await saveMessagesToDB(stateObject.messages);
                }
            ...
            if (ext == 'pdf') {
                return `
                    <a target="_blank" class="d-inline-block m-1" title="${attachment.file_name}" href="${atob(window.ipbxapi_host)}/storage/${localStorage.auth_token}/${attachment.id}/${attachment.file_name}">
                        <div class="align-items-center bg-body-tertiary d-flex flex-column fs-1 justify-content-center rounded-3" style="min-height: 200px; min-width: 200px">
            ...
            if (!data.threads.items.length) {
                try {
                    ...
                    if (fetchData.data) {
                        ...
                        saveThreadToDB(cloneObjFromProxy(fetchData.data.items), data.threads.hasMore);
                        ...
                        fetchData.data.items.forEach(thread => {
                            ...
                            authPhonesArray.forEach(authPhone => {
                                ...
                                //if thread not found then push to items else update the thread
        ...
        async function loadMoreThreads() {
            ...
            try {
                ...
                if (phoneId === 0) {
                    ...
                    fetchData = {
                        data: {
                            items: await getItemsFromDB("threads", "order_by_lmsg", lastMsgId)
                        }
                    }
                ...
                // saveThreadToDB(cloneObjFromProxy(threads), hasMore);
        ...
        async function loadMoreConversations() {
            ...
            try {
                ...
                const thread = data.messages[fetchData.data.thread_id];
        ...
        async function updateMessages(messages) {
            ...
            for (const msg of messages) {
                ...
                let findThread = data.threads.items.find(thread => thread.thread_id == msg.thread_id);
                let unreadCount = findThread ? findThread.unread : 0;

                if (data.selectedThreadId == msg.thread_id && findThread) {
                    findThread.unread = 0;
                } else {
        ...
        $(async function() {
            ...
            $(document).on('reef:render', function(e) {
                ...
                saveMessagesToDB(cloneObjFromProxy(data.messages));
        ...
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.register('/service.js')
                .then(function(registration) {
                    ...
                    // Check if local storage has the auth_token
                    let auth_token = localStorage.auth_token;

                    // Ensure the service worker is active
                    if (registration.active) {
                        if (auth_token) {
                            console.log('auth_token sent to service worker');
                            registration.active.postMessage({
                                api_url: window.ipbxapi_host,
                                auth_token: auth_token
                            });
                        } else {
                            console.log('No auth_token found in local storage.');
                        }
                    } else {
            ...
            navigator.serviceWorker.addEventListener('message', function(event) {
                ...
                // main thread list update
                const thread = data.threads.items.find(item => item.thread_id == res.thread_id);

                const threadIndex = data.threads.items.findIndex(item => item.thread_id == res.thread_id);

                let unreadCount = thread ? thread.unread : 0;

                if (data.selectedThreadId == res.thread_id && thread) {
                ...
                res.unread = unreadCount;

                if (thread) {

                    if (thread.message_id < res.message_id) {
                        // update thread with new message thread
                        data.threads.items[threadIndex] = res;
