...
<body>
    ...
    <div class="layout-wrapper d-md-flex">
        ...
        <div class="side-menu flex-md-column px-lg-1 ms-lg-0">
            <!-- LOGO -->
            ...
            <div class="px-3 pt-4">
                <div class="align-items-center d-flex justify-content-between mb-4">
                    ...
                    <div class="d-flex">
                        ...
                        <button onclick="newConversation()" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Send New Message"> <i class="ri-chat-new-line"></i> New</button>
                    </div>
                </div>
    ...
    <script>
        ...
        async function loadFromDB() {
            ...
            return {
                ...
                messages: await getMessagesFromDB(0),
                phoneData: phoneData ?? {
                    selected: 0,
                    threads: {},
                }
        ...
        async function getMessagesFromDB(upper, threadID = 0, limit = 50) {
            ...
            while (cursor) {
                ...
                if (cursor.value.id < upper) {
                    ...
                    if (cursor.key == threadID && values[threadID].items.length < limit) {
                ...
                if (!upper) {
                    ...
                    if (!values[cursor.key]) {
                        values[cursor.key] = {
                            items: [],
                            thread_id: cursor.key,
                            hasMoreMessages: true
                        };
                    }

                    if (values[cursor.key].items.length < limit) {
                        // Key already exists, so push the value under the existing key
                        values[cursor.key].items.push(cursor.value);
                    }
    ...
    <script>
        ...
        const services = JSON.parse(localStorage.user).services;
        ...
        function selectPhoneComponent() {
            ...
            const authPhonesArray = data.selectedThreadInfo.participants.filter(participant => {
        ...
        function conversationHeadTemplate() {
            ...
            let myNumber = data.selectedThreadInfo.type == 'single' ? data.selectedThreadInfo.participants?.find(participant => {
                return activeServicesByType.sms.find(service => service.phone_id === participant.phone_id);
            })?.number : '';
        ...
        function phoneNumberTemplate() {
            ...
            return smsService[0] ? '<div class="me-2 p-1 rounded-2">' + smsService[0]?.number + '</div>' : '';
            ...
            try {
                if (!threadExist) {
                    ...
                    if (fetchData.data.messages.items.length) {
                        ...
                        saveMessagesToDB(cloneObjFromProxy(fetchData.data.messages.items));
        ...
        async function loadMoreConversations() {
            ...
            try {
                ...
                fetchData = {
                    data: {
                        messages: (await getMessagesFromDB(lastMsgId, threadId))[threadId] ?? [],
                        thread_id: threadId
                    }
                }

                if (fetchData.data.messages.length === 0 || fetchData.data.messages.items.length === 0) {
