...
async function loadAndInitializeState() {
    ...
    try {
        ...
        if (Object.keys(localState).length !== 0) {
            ...
            await saveDataToDB("phoneData", stateObject.phoneData);
        }
...
async function loadMoreThreads() {
    ...
    try {
        ...
        if (phoneId === 0) {
            ...
            fetchData = {
                data: {
                   items: await getItemsFromDB("threads", "service_type, msg_id", range),
                }
            }
...
$(async function () {
    ...
    function initializeIntlTelInput(inputField) {
        return intlTelInput(inputField, {
            utilsScript: "https://cdn.jsdelivr.net/npm/intl-tel-input@18.2.1/build/js/utils.js",
            initialCountry: "us",
            separateDialCode: true,
        ...
        $("#addRecipientField").click(function () {
            let newRecipientField = `<input type="tel" class="form-control bg-light border-light mb-2" name="recipient" required>`;
            $("#recipientFieldsContainer").append(newRecipientField);

            // Initialize intlTelInput for the newly added input field
            initializeIntlTelInput($("#recipientFieldsContainer input[name='recipient']").last()[0]);
        });


    // Initialize intlTelInput for the initial recipient input field
    initializeIntlTelInput($('[name="recipient"]')[0]);
    ...
    $(document).on('reef:render', function (e) {
        ...
        saveDataToDB("phoneData", cloneObjFromProxy(data.phoneData));
