...
function threadsListTemplate() {
    ...
    return `${threads.map(function (thread, index) {
      ...
      data.unreadMessageCounts[thread.thread_id] = thread.unread;

      let unreadBadge = data.unreadMessageCounts[thread.thread_id] > 0 ? `<span class="badge badge-soft-danger rounded-pill">${data.unreadMessageCounts[thread.thread_id]}</span>` : "";
...
async function fetchThreads() {
    ...
    if (!data.threads.items.length) {
        try {
            ...
            let fetchData = await _request("/v1/arc/list");
    ...
    try {
        ...
        if (phoneId === 0) {
            ...
            if (fetchData.data.items.length === 0) {
                ...
                fetchData = await _request("/v1/arc/list", 'GET', {
                    action: 'messages',
                    startId: lastMsgId
                });
            ...
            fetchData = await _request("/v1/arc/list" + "/" + phoneId, 'GET', {
                action: 'messages',
                startId: lastMsgId
            });
        }
...
async function fetchConversations(threadId) {
    ...
    try {
        if (!threadExist) {
            ...
            const fetchData = await _request("/v1/arc/index" + "/" + threadId);
...
async function loadMoreConversations() {
    ...
    try {
        ...
        const fetchData = await _request("/v1/arc/index" + "/" + threadId, 'GET', {
            startId: lastMsgId,
            action: 'messages'
        });
...
async function fetchLatestMessages() {
    ...
    try {
        ...
        const resp = await _request("/v1/arc/latest", 'GET', {
            startId: lastId
        });
...
async function newConversationSubmit(e) {
    ...
    try {
        const resp = await _request("/v1/arc/create", 'POST', dataObj);
...
async function updateMessages(messages) {
    ...
    for (const msg of messages) {
        ...
        if(data.selectedThreadId == msg.thread_id){
            ...
            unreadCount = data.unreadMessageCounts.hasOwnProperty(msg.thread_id) ? data.unreadMessageCounts[msg.thread_id] += 1 : data.unreadMessageCounts[msg.thread_id] = 1;
        }
...
async function fetchPhoneThreads(phoneId) {
    const threads = await _request("/v1/arc/list" + "/" + phoneId, 'GET', {
        action: 'messages',
