...
async function loadMoreThreads() {
    ...
    const lastMsgId = threads.length > 0 ? threads[threads.length - 1].last_message_id : 0;
        ...
        if (data.selectedThreadId) {
            ...
            if (msg_ids) {
                ...
                filteredMessages.forEach(item => {
                    data.threads.items[data.threads.items.findIndex(item => item.thread_id == data.selectedThreadId)].unread = false;
                });
            }
...
async function openSingleChat(id) {
    ...
    if (filteredMessages.length > 0) {
        makeReadMessageRequest(filteredMessages[0].last_message_id);
    }

    // set unread = false for openned thread
    data.threads.items[data.threads.items.findIndex(item => item.thread_id == id)].unread = false;
...
async function replyMessage(e) {
    ...
    try {
        ...
        data.threads.items.sort((a, b) => b.last_message_id - a.last_message_id);
        ...
        if (data.phoneData.selected !== 0) {
            ...
            selectedPhoneThread.last_message_id = resp.data.new_message.msg_id;
            ...
            data.phoneData.threads[data.phoneData.selected].sort((a, b) => b.last_message_id - a.last_message_id);
...
async function updateMessages(messages) {
    ...
    for (const msg of messages) {
        ...
        // sort phoneData by last_message_id
        if (data.phoneData.selected !== 0) {
            data.phoneData.threads[data.phoneData.selected].sort((a, b) => b.last_message_id - a.last_message_id);
        }
...
$(async function () {
    ...
    const threadPhone = $('#threadsPhone');
    const newConversationteModal = $('#newConversationteModal');
    const createConversation = $('#createConversation');
    ...
    threadPhone.val(data.phoneData.selected);

    threadPhone.on("change", function () {
    ...
    //on messages__history scroll 200px from top
    detectScrollToOffset('#messages__history', 500, 'top', () => {
